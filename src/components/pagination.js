...
class pagination extends React.Component {
    ...
    componentDidUpdate(){
        if(this.props.length!=this.state.quantity){
            this.setState({quantity:this.props.length});
        }
        if(this.state.pages.length!=this.state.quantity){
            this.setState({pages:Array.from({length: this.state.quantity}, (_, i) => i + 1)});
        }   
    }
    render() {
        return (
            <div className="mx-auto w-fit text-blue-500 pb-2">
                <ul className="rounded-md border inline-flex border-gray-200">
                    ...
                    <li className="p-2 w-10 h-10 text-center"><Link to={"/"+this.props.param+"?page="+(this.props.currentPage+1)}><FontAwesomeIcon icon={faAngleRight}/></Link></li>
                </ul>
            </div>
        );
    }
